...
class ICASSP2025Dataset(data.Dataset):
    """
    ICASSP2025 Dataset for Task 1
    Input: RGB images (first 2 channels only) from Inputs/Task_1_ICASSP
    Output: Single channel images from Outputs/Task_1_ICASSP
    All images are upsampled to 256x256
    """
    def __init__(
        self,
        data_root,
        ...
        **kwargs
        ...
        self.interpolation_mode = interpolation_map.get(interpolation.lower(), T.InterpolationMode.BILINEAR)
        
        self.input_folder = self.data_root / "Inputs" / "Task_1_ICASSP"
        self.boundaries_folder = self.data_root / "BoundaryMaps" 
        self.output_folder = self.data_root / "Outputs" / "Task_1_ICASSP"
        
        self.input_paths = []
        for ext in ['jpg', 'jpeg', 'png', 'bmp', 'tiff']:
            self.input_paths.extend(list(self.input_folder.glob(f'*.{ext}')))
            self.input_paths.extend(list(self.input_folder.glob(f'*.{ext.upper()}')))
        
        print(f'Found {len(self.input_paths)} input images')
        
        print(f'Using interpolation: {interpolation}')
        
        self.transform = T.Compose([
            T.Resize(image_size, interpolation=self.interpolation_mode),
            # T.RandomHorizontalFlip() if augment_horizontal_flip else nn.Identity(),
            T.ToTensor()
        ])
        ...
        for input_path in self.input_paths:
            ...
            if output_path.exists():
                self.valid_pairs.append((input_path, output_path))
            else:
                print(f'Warning: Output file not found for {input_path.name}')
        
        print(f'Found {len(self.valid_pairs)} valid input-output pairs')
    
    def __len__(self):
        return len(self.valid_pairs)
    
    def __getitem__(self, index):
        input_path, output_path = self.valid_pairs[index]
        
        
        input_img = Image.open(input_path).convert('RGB')
        
        output_img = Image.open(output_path).convert('L')
        shape = output_img.size  

        
        boundary_filename = f"boundary_{input_path.name}"
        boundary_path = self.boundaries_folder / boundary_filename

        if self.use_boundary:
            if boundary_path.exists():
                boundary_img = Image.open(boundary_path).convert('L')
            else:
                print(f'Warning: Boundary file not found: {boundary_path}')
                
                boundary_img = Image.new('L', input_img.size, 0)
        
            boundary_img = self.transform(boundary_img)

            
            input_img = self.transform(input_img)
            output_img = self.transform(output_img)
        
            if boundary_img.dim() == 3:
                boundary_channel = boundary_img[0:1, :, :]  
            else:
                boundary_channel = boundary_img.unsqueeze(0)  
                
            input_channels = input_img[:2, :, :]  
            input_channels = torch.cat([input_channels, boundary_channel], dim=0)
        
        else:
            input_img = self.transform(input_img)
            output_img = self.transform(output_img)

            input_channels = input_img[:3, :, :]
        ...
        # [-1, 1]
        if self.normalize_to_neg_one_to_one:
            input_channels = normalize_to_neg_one_to_one(input_channels)
            output_img = normalize_to_neg_one_to_one(output_img)
        
        return {
            'image': output_img,      # [1, 256, 256] 
            'cond': input_channels,   # [3, 256, 256] 
