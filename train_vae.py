...
class Trainer(object):
    def __init__(
        ...
        lr_lambda = lambda iter: max(max(0, (1 - iter / train_num_steps)) ** 0.95, min_lr/train_lr)
                ...
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        ...
                        if ga_ind == 0:
                            ...
                            # 初始化判别器相关变量
                            disc_loss = log_dict.get("train/disc_loss", 0.0)
                            logits_real = log_dict.get("train/logits_real", 0.0)
                            logits_fake = log_dict.get("train/logits_fake", 0.0)
                        else:
                            self.opt_disc.zero_grad()
                            self.accelerator.backward(loss)
                            self.opt_disc.step()
